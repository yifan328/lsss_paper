...
# This script is used to generate the differences between optimal and suboptimal for msprime.
...
def get_switch_error(focal, panel, sample_size, length, seed):
    '''
    Compute the switch error of the sub-/optimal. Change the value of beta for sub-/optimal
    '''
    ts = msp.simulate(
...
def main():
   ...
   # results = [get_switch_error_std(focal, panel, sample_size, len_multiplier, i) for i in np.arange(array*N, (array+1)*N)]
    ...
    with open('switch_error/switch_error%s.pkl' % array, 'wb') as f:
        pickle.dump(results, f)
