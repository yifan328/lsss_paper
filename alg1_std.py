...
def get_imputation_error_std(focal: int, panel: List[int], sample_size: int, len_multiplier: float, k: float, seed: int):
...
def main():
    ...
    length = 1e8
    len_multiplier = 0.413
    # k = MAF
    k = 0.05
    results = get_imputation_error_std(focal, panel, sample_size, len_multiplier, k, 65)
    with open('results_alg1_std/beta_y.pkl', 'wb') as f:
